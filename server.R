...
server <- function(input, output, session) {
  ...
  efec <- dplyr::filter(dfunda,Instrumento=="REPORTO")
 ...
 e <- dplyr::filter(dfunda,Instrumento=="EFECTIVO")
  ...
  observeEvent(input$summit,{
    ...
    #Vendiste de mas
    error2 <- c()
    ddfunda <- dfunda %>% dplyr::filter(Fondo == input$fondo)
  ...
  observe({
    ...
    fondo <- dfunda %>%
      dplyr::filter(!(Instrumento %in% especiales))
  ...
  warnings <- eventReactive(input$summit,{
    ...
    f <- c()
    funddb <- fundb %>% dplyr::filter(Fondo == selected_fund & !(Instrumento %in% excepciones))
    advert <- c(" ")
